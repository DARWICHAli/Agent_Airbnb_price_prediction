...
def basic_clean(df, cfg):
    ...
    # drop rows with missing / non-positive prices, then clip extreme outliers
    df = df[df[price_col].notna() & (df[price_col] > 0)].copy()
    clip_q = prep.get("price_clip_quantile", 0.99)
    upper = df[price_col].quantile(clip_q)
    df[price_col] = df[price_col].clip(upper=upper)
    ...
    for c in df.select_dtypes(include=["object"]).columns:
        ...
        if c == "host_since":
            ...
            # drop the raw date column; the derived feature is what the model uses
...
if __name__ == "__main__":
    ...
    for f in sorted(os.listdir(raw_dir), reverse=True):
        if "listings" in f and args.city.lower() in f.lower() and not f.endswith(".gz"):
    ...
    print(f"Preprocessing done. {len(df_clean)} rows -> {len(train)} train / {len(val)} val / {len(test)} test")
